    ...
    public partial class ReportBankBook : System.Web.UI.Page
    {
        ...
        public void showGrid()
        {
            ...
            SqlCommand cmd = new SqlCommand(" SELECT '" + FdT + "' AS FromDate, '" + TdT + "' AS ToDate, GL_MASTER_1.TRANSTP, CONVERT(nvarchar(10), GL_MASTER_1.TRANSDT, 103) AS TRANSDT, GL_MASTER_1.TRANSDT AS TR, " +
                      ...
                      " CASE WHEN GL_MASTER_1.DEBITAMT > 0 THEN 'TO ' + dbo.GL_ACCHART.ACCOUNTNM + ((CASE WHEN GL_MASTER_1.CHEQUENO != '' THEN ' Chq #' + GL_MASTER_1.CHEQUENO + ' ' + CONVERT(nvarchar(20), GL_MASTER_1.CHEQUEDT, 105) WHEN GL_MASTER_1.CHEQUENO = '' THEN '' ELSE '' END)) WHEN GL_MASTER_1.CREDITAMT > 0 THEN 'BY ' + dbo.GL_ACCHART.ACCOUNTNM + ((CASE WHEN GL_MASTER_1.CHEQUENO != '' THEN '<br/>&nbsp; Chq #' + GL_MASTER_1.CHEQUENO + ' ' + CONVERT(nvarchar(20), GL_MASTER_1.CHEQUEDT, 105) WHEN GL_MASTER_1.CHEQUENO = '' THEN '' ELSE '' END))  END + ' <br/> &nbsp;' +  GL_MASTER_1.REMARKS AS CreditNM, " +
