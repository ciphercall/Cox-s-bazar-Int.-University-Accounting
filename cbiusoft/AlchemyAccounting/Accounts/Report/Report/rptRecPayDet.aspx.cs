...
namespace AlchemyAccounting.Accounts.Report.Report
{
    public partial class rptRecPayDet : System.Web.UI.Page
    {
        ...
        public void showGrid_OP()
        {
            ...
            SqlCommand cmd = new SqlCommand(" SELECT A.TRANSDT, A.TRANSNO, (CASE WHEN TABLEID = 'GL_STRANS' THEN SUBSTRING(TABLEID, 4, 1) WHEN TABLEID = 'GL_MTRANS' THEN SUBSTRING(TABLEID, 4, 1) " +
        ...
        public void showGrid_Jour()
        {
            ...
            if (conn.State != ConnectionState.Open)conn.Open();
            SqlCommand cmd = new SqlCommand(" SELECT * FROM (SELECT CONVERT(NVARCHAR(20), GL_MASTER.TRANSDT, 103) AS TRANSDT, GL_MASTER.TRANSDT AS TD, (CASE WHEN GL_MASTER.TABLEID = 'GL_STRANS' THEN SUBSTRING(GL_MASTER.TABLEID, 4, 1) WHEN GL_MASTER.TABLEID = 'GL_MTRANS' THEN SUBSTRING(GL_MASTER.TABLEID, 4, 1) " +
