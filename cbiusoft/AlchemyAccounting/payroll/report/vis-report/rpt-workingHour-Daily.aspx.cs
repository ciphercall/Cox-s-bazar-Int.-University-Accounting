...
{
    public partial class rpt_workingHour_Daily : System.Web.UI.Page
    {
        ...
        private void ShowGrid()
        {
            ...
            SqlCommand cmd = new SqlCommand("SELECT   HR_EMP.EMPNM, HR_HOUR.NORMALHR, HR_HOUR.NORMALOT, HR_HOUR.FRIDAYOT,(HR_HOUR.NORMALHR + HR_HOUR.NORMALOT + HR_HOUR.FRIDAYOT)AS TOTALHOUR, GL_COSTP.COSTPNM " +
                                           " FROM HR_EMP INNER JOIN " +
                       " HR_HOUR ON HR_EMP.EMPID = HR_HOUR.EMPID INNER JOIN " +
                     " GL_COSTP ON HR_HOUR.COSTPID = GL_COSTP.COSTPID " +
                        " WHERE HR_HOUR.TRANSDT= @FROMDATE ", conn);
