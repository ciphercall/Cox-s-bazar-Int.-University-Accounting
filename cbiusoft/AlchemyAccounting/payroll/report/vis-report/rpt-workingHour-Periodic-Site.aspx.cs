...
{
    public partial class rpt_workingHour_Periodic_Site : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                if (!IsPostBack)
                {
                    ...
                    string site = Session["SiteID"].ToString();
                    ...
                    Global.lblAdd("select COSTPNM from GL_COSTP where COSTPID='"+site+"'",lblSiteNM);
        ...
        private void ShowGrid()
        {
            ...
            SqlCommand cmd = new SqlCommand("SELECT     HR_EMP.EMPNM, SUM(HR_HOUR.NORMALHR) AS NORMALHR, SUM(HR_HOUR.NORMALOT) AS NORMALOT, SUM(HR_HOUR.FRIDAYOT) AS FRIDAYOT, ( SUM(HR_HOUR.NORMALHR) + SUM(HR_HOUR.NORMALOT) + SUM(HR_HOUR.FRIDAYOT)) AS TOTALHOUR " +
                                            "FROM HR_EMP INNER JOIN "+
                       " HR_HOUR ON HR_EMP.EMPID = HR_HOUR.EMPID INNER JOIN " +
                       " GL_COSTP ON HR_HOUR.COSTPID = GL_COSTP.COSTPID "+
                        " WHERE HR_HOUR.TRANSDT BETWEEN @FROMDATE AND @TODATE AND HR_HOUR.COSTPID='" + site + "' " +
                        " GROUP BY  HR_EMP.EMPNM ", conn);
