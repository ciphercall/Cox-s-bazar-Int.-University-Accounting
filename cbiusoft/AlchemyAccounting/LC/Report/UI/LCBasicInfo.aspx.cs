...
namespace AlchemyAccounting.LC.Report.UI
{
    public partial class LCBasicInfo : System.Web.UI.Page
    {
        ...
        public static string[] GetCompletionListLC(string prefixText, int count, string contextKey)
        {
            ...
            SqlCommand cmd = new SqlCommand("SELECT LCNO FROM LC_BASIC where LCNO LIKE '" + prefixText + "%'", conn);
        ...
        {
            lblLCCD.Text = "";
            string LcAccNM = "L/C NO : " + txtLCID.Text;
            Global.lblAdd(@"SELECT ACCOUNTCD FROM GL_ACCHART WHERE substring(ACCOUNTCD,1,7) in ('4010103','2020103') AND STATUSCD='P' AND ACCOUNTNM = '" + LcAccNM + "'", lblLCCD);
            btnSearch.Focus();
        }
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                Page.ClientScript.RegisterStartupScript(
                    this.GetType(), "OpenWindow", "window.open('../Report/rptLcBasicInfo.aspx','_newtab');", true);
