...
{
    public partial class Process_Rec : System.Web.UI.Page
    {
        ...
        public void ShowGrid_Transaction()
        {
            ...
            ISNULL(EIM_TRANSMST.POBANKBR,'')  AS REMARKS
            ...
            WHERE     (EIM_TRANS.TRANSDT = @TRANSDT AND EIM_TRANS.TRANSTP = 'JOUR' AND TRANSFOR='RECEIVABLE') 
            GROUP BY  EIM_TRANS.TRANSYY, EIM_TRANS.TRANSNO, EIM_TRANS.CNBCD, (EIM_TRANS.STUDENTID + '  ' + EIM_SEMESTER.SEMESTERNM + '  ' + EIM_PROGRAM.PROGRAMNM + '  ' + ISNULL(EIM_TRANS.REMARKS,'') + '  ' + ISNULL(PONO,'') +'  '+
            (CASE WHEN EIM_TRANSMST.PODT='1999-01-01' THEN '' ELSE CONVERT(NVARCHAR(20),EIM_TRANSMST.PODT,103) END) +'  ' + ISNULL(EIM_TRANSMST.POBANK,'') + '  ' + 
        ...
        public void Transaction_process()
        {
            ...
            foreach (GridViewRow grid in gv_Receivable.Rows)
            {
                try
                {
                    ...
                    {
                        lblSerial_JOUR.Text = "";
                        Global.lblAdd(@"Select max(SERIALNO) FROM  GL_MASTER where TRANSDT = '" + trans_DT + "' and TRANSTP = 'JOUR' AND TABLEID ='EIM_TRANS' and SERIALNO like '11%' ", lblSerial_JOUR);
